...
public class StringProcessor {
  ...
  public static String idProcessing(String firstName, String lastName, String petName, int year) {
    ...
    if (year < 1000 || year > 2021) {
      ...
      if (firstName.length() < 2 || firstName.length() > 26) {
        throw new IllegalArgumentException("First Name: invalid length.");
      }
      if (lastName.length() < 2 || lastName.length() > 26) {
        throw new IllegalArgumentException("Last Name: invalid length.");
      }
      if (petName.length() < 2 || petName.length() > 26) {
        throw new IllegalArgumentException("Pet Name of invalid length.");
      }
  ...
  public String secretCode(int offset) {
    ...
    if (offset < 0) {
      throw new IllegalArgumentException("Invalid Offset: Negative offset!");
    }
