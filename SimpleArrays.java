...
class SimpleArrays{
	...
	public String arrayConcat(int index) {
		...
		if(index<0 || index>array.length-1) { throw new ArrayIndexOutOfBoundsException("Index out of Bounds!"); }

		for(int i=index; i<array.length; i++) { concatenatedString+=array[i]+"#"; }
	...
	public String arrayCrop(int start, int end) {
		...
		if (start<0 || start>array.length-1 || end<0 || end>array.length-1) { return "Fail"; }
		else if (start==end) { return "Match"; }
		else if (end<start) {
			int temp = start;
			start = end;
			end = temp;
