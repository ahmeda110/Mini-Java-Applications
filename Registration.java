...
public class Registration {
  ...
  public void initializeConnection() {
    try {
      compConnect = DriverManager.getConnection(DBURL, USERNAME, PASSWORD);
  ...
  public void insertNewCompetitor(String id, String lName, String fName, int age, String instrument, String teachName) throws IllegalArgumentException {
    if (age < 5 || age > 18) {
      throw new IllegalArgumentException("Invalid competior age!");
    }
    ...
    try {
      ...
      while (results.next()) {
        array.add(results.getString("TeacherID"));
      }
      if (!array.contains(teachName)) {
        throw new IllegalArgumentException("Teacher id not found in the database.");
      }
    } catch (SQLException e) {
      e.printStackTrace();
    }
  ...
  public void registerNewTeacher(String teachId, String lName, String fName, String phone, String studioName, String studioPhone, String address) {
    try {
      ...
      if (!array.contains(studioName)) {
        insertNewStudio(studioName, studioPhone, address);
      }
    } catch (SQLException e) {
      e.printStackTrace();
    }
    ...
    try {
      ...
      while (results.next()) {
        if (results.getString("TeacherID").equals(teachId)) {
